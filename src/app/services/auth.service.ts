...
export class AuthService {
  ...
  private extractAuthResponse(raw: any): AuthResponse {
    ...
    // Map any legacy backend role values to frontend UserRole
    const rawRole = String(p.role ?? 'CLIENT').toUpperCase();
    const roleMap: Record<string, UserRole> = {
      'USER':     'CLIENT',   // legacy fallback
      'CUSTOMER': 'CLIENT'    // removed — backend now uses CLIENT directly
    };
    ...
    const user: User = {
      ...
      name:          p.name ?? p.fullName ?? p.username,  // fullName added — Spring returns getFullName()
      username:      p.username,
      email:         p.email,
      phone:         p.phone,
      address:       p.address,
      ...
      role,
      avatar:        p.avatar   ?? 'avatar.png',          // fallback to default avatar
      bio:           p.bio,
