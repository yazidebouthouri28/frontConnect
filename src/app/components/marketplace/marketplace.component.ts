...
export class MarketplaceComponent implements OnInit {
  ...
  loadProducts(): void {
    ...
    this.productService.getAll(0, 100).subscribe({
      next: (apiProducts) => {
        ...
        console.log('Is array:', Array.isArray(apiProducts));
        console.log('Length:', apiProducts?.length);
        if (Array.isArray(apiProducts) && apiProducts.length > 0) {
          this.originalProducts = apiProducts.map((p: any) => {
            const mapped = this.mapApiProduct(p);
  ...
  private mapApiProduct(apiProduct: any): Product {
    return {
      ...
      image: apiProduct.images?.[0] || apiProduct.thumbnail || 'https://images.unsplash.com/photo-1478131143081-80f7f84ca84d?w=800&q=80',
      price: apiProduct.price,
      rentalPrice: apiProduct.isRentable ? apiProduct.rentalPricePerDay : undefined,
      rating: apiProduct.rating || 4.5,
      reviews: apiProduct.reviewCount || Math.floor(Math.random() * 200) + 50,
      inStock: (apiProduct.stockQuantity || 0) > 0 || true,
      ...
      discount: apiProduct.originalPrice ? Math.round((1 - apiProduct.price / apiProduct.originalPrice) * 100) : undefined
    };
  }
