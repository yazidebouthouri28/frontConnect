...
export class MarketplaceComponent {
  ...
  // Notification
  notification: { show: boolean; message: string; type: string } = { show: false, message: '', type: '' };
  ...
  products: Product[] = [
    {
      ...
      options: [{ name: 'Color', values: ['Forest Green', 'Sandstone'] }]
    },
    {
      id: 2,
    ...
    },
    {
      ...
      options: [{ name: 'Color', values: ['Tan', 'White', 'Charcoal'] }],
      reviewsList: [
  ...
  addToCart(product: Product) {
    ...
    this.cartService.addToCart(cartItem).subscribe({
      ...
      error: (err) => {
        ...
        this.showNotification(`Erreur: ${err.message || 'Impossible d\'ajouter au panier'}`, 'error');
      }
    });
  }
