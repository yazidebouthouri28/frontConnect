...
export class SellerComponent implements OnInit {
    ...
    if (confirm('Delete this product?')) {
      ...
      this.productService.delete(String(id)).subscribe({
        next: (res) => {
          ...
          // Reload after short delay to sync with backend
          setTimeout(() => this.loadProducts(), 1000);
        },
        error: (err) => {
          console.error('[deleteProduct] Error:', err);
          alert('❌ Failed to delete: ' + (err.error?.message || err.message || 'Unknown error'));
        }
      });
    }
  }
  ...
  deleteCategory(id: string) {
    if (confirm('Delete this category? This will also affect associated products.')) {
