...
export class CandidatureManageComponent implements OnInit {
    ...
    loadData(): void {
        ...
        this.loading = true;
        forkJoin({
            candidatures: this.candidatureService.getByEvent(this.eventId),
    ...
    accept(id: number): void {
        this.candidatureService.updateStatus(id, 'ACCEPTEE').subscribe({
            next: () => this.loadData()
        });
    }

    refuse(id: number): void {
        this.candidatureService.updateStatus(id, 'REJETEE').subscribe({
            next: () => this.loadData()
        });
    }

    waitlist(id: number): void {
        this.candidatureService.updateStatus(id, 'EN_ATTENTE').subscribe({
            next: () => this.loadData()
        });
    }
}
